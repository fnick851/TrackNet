    ...
    token.selectAll("circle")
         ...
         .each("end", function() {
             ...
             if (intro_step == 2) {
				 ...
				 intro_step = 3;
				 tooltip = $(token.select(".bubbleEdge")[0]).qtip({
					 content: {
						 title: 'Instruction: Bubbles',
		                 text: '<ul>\
		                    	    <li>When a bubble is closed, it shrinks to an inner position.\
		                    	        You can simply reactivate it by clicking on\
		                    	        the shrunk bubble.</li>\
		                    	    <br>==> Reactivate this blue bubble.​\
		                    	</ul>'
		             },
		             position: {
		                 my: 'bottom center',
           ...
           if (!data.shrink) {
               ...
               .each("end", function() {
                   ...
                   if (intro_step == 1) {
    				   tooltip.qtip('destroy');
    				   intro_step = 2;
    				   tooltip = $("#categoryTooltip").qtip({
    					   content: {
    						   title: 'Instruction: Bubbles',
    		                   text: '<ul>\
    		                    	      <li>The bubble expands and a tooltip displaying the top trackers\
    		                    	          of that type appears.</li>\
    		                    	​      <li>A close button also shows up.\
    		                    	          You can close this bubble if you are not interested in\
    		                    	          this type of trackers. TrackNet remembers which types\
    		                    	          are inactivated.​</li>\
    		                    	      <br>==> Close this blue bubble.​\
    		                    	  </ul>'
    		               },
    		               position: {
    		                   my: 'left center',
